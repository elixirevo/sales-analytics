...
컨테이너 기본 명령은 `serve`입니다. 프로세스가 시작되면 기본적으로 최근 5년 범위에서 최초 거래일을 찾고, 그 날짜부터 어제 영업일까지 백필해 DB에 적재합니다. Toss 주문 API 제한에 맞춰 탐색 시작일은 `2022-01-01`보다 과거로 내려가지 않습니다. 이미 `SUCCESS`인 매장/영업일은 건너뛰므로 컨테이너 재시작 시 중복 수집하지 않습니다.

초기 백필은 과거 데이터를 DB에 적재하고 월/연 기간 CSV를 갱신합니다. 과거 날짜별 일별 CSV와 주차별 CSV는 업로드하지 않습니다. 초기 백필이 끝난 뒤에는 프로세스가 계속 실행되면서 매장별 `business_close_time + SCHEDULER_CLOSE_DELAY_MINUTES`가 지난 영업일을 찾아 한 번씩 배치를 실행합니다.
